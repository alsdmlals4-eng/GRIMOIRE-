# 스펠 v6 콘셉트 재기획 기준선

## 문서 상태

- 프로젝트: `스펠` (임시)
- 저장소: `alsdmlals4-eng/Spell`
- 제품 단계: `CONCEPT_APPROVAL`
- 실행 프로필: `PLANNING_ONLY_PROFILE`
- Work Mode: `PLAN`
- 상태: `GATE_1_WORKING_BASELINE`
- 작성일: 2026-07-27
- 기준 브랜치: `gpt/planning-spell-20260725`
- 프로젝트 코어 전체: `CORE_CANDIDATE`
- 프로젝트 코어 경계: `PROJECT_CORE_BOUNDARY_CONFIRMED_FOR_GATE_1`
- Gate 1 전체: `UNVERIFIED`
- 구현 상태: `NOT_STARTED`
- CORE_POC: `NOT_AUTHORED`
- Vertical Slice: `BLOCKED_BY_GATE_1`

## 1. 권한과 자료 지위

### 최상위 원본 계약

- 사용자 제공 `VERTICAL_SLICE_MASTER_REFERENCE_v6.md`
- `docs/contracts/VERTICAL_SLICE_EXECUTION_PROMPT_SHORT_v6.md`
- `docs/contracts/VERTICAL_SLICE_MASTER_REFERENCE_v6.md`의 버전·해시 Manifest

v6는 기획 순서, Skill 오케스트레이션, Gate, 증거, 검증과 상태 표현을 책임진다.

### Gate 1 책임 원본

1. 이 문서 — Gate 1 통합 기준선과 진행 상태
2. `docs/planning/GATE_1_GAMEPLAY_LOOP_SYSTEM.md` — Micro·Combat·Session·Semester Loop
3. `docs/planning/GATE_1_EXPERIENCE_CURVE_SYSTEM.md` — 첫 5분부터 첫 숙련 자동화
4. `docs/planning/GATE_1_PROJECT_CORE_BOUNDARY_SYSTEM.md` — 비타협 코어·지원층·외피
5. `docs/planning/DECISION_LOG.md` — 사용자 결정과 상태 인덱스
6. `docs/ACTIVE_CONTEXT.md` — 현재 작업 상태
7. `docs/DESIGN_DOCUMENT_REGISTRY.json` — 단일 책임 원본 라우팅

### 기존 Spell 기획 문서

기존 `docs/planning/` 문서는 아이디어·사용자 결정 이력·후보 시스템·수치·폐기 이력·벤치마킹·제작 위험을 제공하는 참조 자료다.

```text
기존 작업물
→ 사실·선호·가설·폐기 이력 추출
→ v6 기준 재검토
→ AMPLIFY / SUPPORT / NEUTRAL / CONFLICT / UNPROVEN
→ 사용자 승인
→ 새 책임 원본 반영
```

기존 문서에 존재한다는 이유만으로 새 Gate 1 확정값으로 자동 승계하지 않는다.

## 2. 승인된 최상위 경험 — GM-CORE-01

사용자 승인: 2026-07-27, `권장안대로 진행해`

상태: `PRIMARY_EXPERIENCE_DIRECTION_CONFIRMED`

> 마법 글자의 의미를 이해하고 상황에 맞는 주문을 설계해 발견하는 지적 마법 판타지.

### 경험 우선순위

1. 조합하고 발견하는 지적 재미
2. 직접 쓰는 마법의 손맛
3. 숙련하고 자동화하는 성장감

2번과 3번은 1번을 약화시키지 않는 범위에서만 확대한다.

### 보조 경험

- 직접 작성: 마법을 내가 실제로 다룬다는 손맛과 주체성
- 숙련·스톡: 이해한 반복을 줄이고 새 조합에 집중시키는 성장 보상
- 소환수: 작성 시간을 확보하고 준비·충전 우선순위를 만드는 보조자
- 마법학교: 새로운 글자·활용 문제·관계·전문화를 공급하는 장기 프레임

## 3. 승인된 목표 플레이어·플레이 상황 — GM-AUDIENCE-01

사용자 승인: 2026-07-27, `권장안대로 진행`

상태: `AUDIENCE_AND_PLAY_CONTEXT_CONFIRMED`

> 집·카페·휴식 시간처럼 잠깐 집중할 수 있는 환경에서 10~15분 동안 하나의 수업·현장실습·마법 발견 단위를 완주하고, 장기적으로 학교생활과 주문 연구를 축적하려는 모바일 플레이어.

```text
Micro 상황: 약 30초~2분
전투·문제 묶음: 약 3~5분
기본 수업·현장실습 세션: 약 10~15분
선택형 상세 감상: 추가 대화·조사·기록으로 자율 연장
```

- 기본 세션은 문제 인지→주문 설계→결과→발견 또는 성장까지 완결한다.
- 전투·대화·보상 사이에 안전한 중단·복귀 지점을 둔다.
- 장시간 플레이를 강제하지 않는다.
- 정확한 시간은 `PLAYTEST_TUNING_REQUIRED`다.

## 4. 승인된 서사 깊이 — GM-NARRATIVE-03

사용자 결정:

> 기본적으론 스토리, 설명을 짧게 하고 대화 선택 시 좀 더 다양하게.

상태: `OPTIONAL_NARRATIVE_DEPTH_CONFIRMED`

### 필수 핵심층

- 현재 상황
- 행동 이유
- 목표
- 위험·실패 조건
- 중요한 선택 결과
- 주문 문제 해결에 필요한 핵심 정보

### 선택형 상세층

- 인물의 감정과 관계
- 학교생활과 일상
- 세계관과 사건 배경
- 마법 글자·주문 원리
- 다른 해결법과 실패 사례
- 소환수 이야기
- 지역·학교 역사

필수 정보는 선택형 상세 대화에만 숨기지 않는다. 상세층을 읽지 않았다는 이유로 불합리한 실패를 만들지 않는다.

## 5. 확정된 플레이어 약속 — GM-PROMISE-01

사용자 승인: 2026-07-27, `확정해`

상태: `PLAYER_PROMISE_CONFIRMED_FOR_GATE_1`

> 마법학교 학생이 되어 글자의 의미를 배우고, 수업과 현장실습에서 주문을 직접 설계해 내가 생각한 해결법으로 세계를 바꾸는 모바일 마법 RPG.

### 설계 필터

```text
배움
→ 글자와 문제의 원리를 이해한다.

설계
→ 상황에 맞는 주문 구조를 선택·작성한다.

세계 변화
→ 설계 결과가 적·환경·사건을 실제로 바꾼다.
```

- 직접 작성은 주체성을 증명하고 반복 노동이 되지 않아야 한다.
- 스톡과 소환수는 설계 판단을 대체하지 않는다.
- 학교 콘텐츠는 완성 주문 지급 메뉴가 아니라 새로운 문법과 문제를 학습시키는 프레임이다.
- 단순 수치 성장·자동 전투·장문 서사가 플레이어 약속보다 전면에 나오지 않는다.

## 6. 확정된 게임플레이 Loop — GM-LOOP-01

사용자 승인: 2026-07-27, `권장안대로 진행`

상태: `LOOP_STRUCTURE_CONFIRMED_FOR_GATE_1`

책임 원본: `docs/planning/GATE_1_GAMEPLAY_LOOP_SYSTEM.md`

```text
수업
→ 마법 글자·원리·설계법 학습

교내 연습
→ 안전한 환경에서 직접 작성·조합·사용

일상
→ 관계·정보·동행·실습 관점 누적

현장실습
→ 실전 문제와 전투에서 응용·변형·발견

복귀·정리
→ 주문·활용법·실패 원인·관계 결과 기록

다음 수업·연구
→ 실전에서 드러난 부족함과 관심 분야 재학습
```

> 수업에서 알고, 연습에서 할 수 있게 되고, 실전에서 다르게 써 보며, 일상에서 그 경험의 관계와 의미가 누적된다.

### 일정 방식

`고정 핵심 사건 + 자유 일정` 혼합형을 채택한다.

- 고정 축: 학기 시작, 필수 기초 수업, 중간 현장실습, 학기 핵심 실습, 시험·평가
- 자유 일정: 선택 수업, 교내 연습, 연구, 실습 준비·조사, 일상·관계, 소환수 교감, 휴식·정비

### Loop 층

- `Micro Loop`: 관찰→메인·보조 선택→직접 작성·스톡→결과 확인→재설계
- `Combat Loop`: 현장 파악→첫 주문→반응 확인→변형·교체→핵심 위협 해결→기록
- `Session Loop`: 수업·연습·현장실습·일상 중 하나를 중심으로 약 10~15분에 완결
- `Semester Loop`: 고정 수업·실습·평가와 자유 일정이 교차하여 다음 학습·관계·문제를 변화

### 수치·범위 상태

- 시간 목표: `PLAYTEST_TUNING_REQUIRED`
- 학기 `자유 일정 10회 + 고정 사건 4회`: `REFERENCE_CANDIDATE`
- 3년·6학기: `CHANGEABLE_SHELL_SCOPE_RISK`
- 전투 중 완전 상태 복원 범위: `NOT_AUTHORED`

## 7. 확정된 경험 곡선 — GM-CURVE-01

사용자 승인: 2026-07-27, `권장안대로 진행`

상태: `EXPERIENCE_CURVE_CONFIRMED_FOR_GATE_1`

책임 원본: `docs/planning/GATE_1_EXPERIENCE_CURVE_SYSTEM.md`

```text
학교 도착·현재 목표 소개
→ 교수의 짧은 시범
→ 첫 메인 글자 직접 작성
→ 세계가 변하는 결과 확인
→ 보조 글자 하나로 작은 설계 선택
→ 짧은 일상·관계 장면
→ 첫 모의 현장실습에서 응용
→ 새 활용 발견·마도서 기록
→ 반복 숙련 후 첫 하위 글자 스톡 해금
```

> 먼저 마법이 작동하는 기쁨을 주고, 곧바로 내가 설계했다는 감각을 주며, 실전에서 응용한 뒤 숙련된 반복만 자동화한다.

### 첫 5분 책임

1. 마법학교 학생과 현재 목표 이해
2. 첫 메인 글자 직접 작성
3. 즉각적인 현상 변화
4. 보조 글자 하나로 결과 변형
5. 플레이어 선택에 따른 다른 결과

첫 5분 종료 시 플레이어는 다음을 이해해야 한다.

- 메인 글자는 핵심 현상을 만든다.
- 보조 글자는 현상의 작동 방식을 바꾼다.
- 직접 작성과 선택이 세계 변화로 이어진다.

### 첫 숙련 자동화

```text
직접 작성
→ 서로 다른 상황에서 반복 사용
→ 역할과 결과 이해
→ 안정적 재현
→ 숙련 인정
→ 숙련 보조 글자의 하위 글자 스톡 후보 해금
```

- 첫 5분에 스톡을 지급하지 않는다.
- 수동 작성 경험 없이 자동화를 먼저 공개하지 않는다.
- 첫 자동화는 하위 글자 스톡부터 시작한다.
- 완성 주문 스톡은 직접 작성·조합·발견을 충분히 이해한 이후로 미룬다.
- 정확한 요구량과 공개 시점은 `PLAYTEST_TUNING_REQUIRED`다.

### 시스템 공개 순서

1. 메인 글자 직접 작성
2. 보조 글자 하나와 결과 차이
3. 마나·작성 시간
4. 현장 목표와 환경 반응
5. 여러 보조 조합과 재설계
6. 하위 글자 스톡
7. 소환수의 전투·수호 지원
8. 소환수의 지정 스톡 충전
9. 완성 주문 스톡과 고급 준비 전술

## 8. 확정된 프로젝트 코어 경계 — GM-CORE-BOUNDARY-01

사용자 승인: 2026-07-27, `권장안대로`

상태: `PROJECT_CORE_BOUNDARY_CONFIRMED_FOR_GATE_1`

책임 원본: `docs/planning/GATE_1_PROJECT_CORE_BOUNDARY_SYSTEM.md`

> 의미를 배운 글자를 직접 쓰고 상황에 맞게 조합하여, 내가 설계한 주문으로 세계를 변화시키고 새 활용을 발견한다.

### 비타협 프로젝트 코어

1. 의미를 가진 마법 글자
2. `메인 글자 1개 + 보조 글자 0개 이상` 조합 문법
3. 신규·미숙·중요 글자의 직접 작성
4. 상황·목표·위험을 읽고 주문을 변형하는 판단
5. 주문 결과가 적·환경·사건을 바꾸는 즉각적이고 설명 가능한 피드백
6. 수업→연습→현장실습→발견·기록→다음 학습의 순환

### 코어 지원 시스템

- 마법학교
- 일상·관계
- 마도서·발견 기록
- 숙련·하위 글자 스톡
- 소환수 전투·수호·지정 생산·충전 지원

지원 시스템은 코어를 증폭하지만 주문 설계 판단을 대신하지 않는다.

### 변경 가능한 외피

- 3년·6학기와 학기 `10+4`
- 학교 규모·지역·등장인물·관계 분기량
- 주인공의 세부 출신
- 수업·실습·일상의 정확한 비율
- 실시간 전투 압박과 집중 모드
- 메인 레벨·보조 운용량·스톡 수치
- 완성 주문 스톡의 존재와 비중
- 소환수 수·편성·성장 방식
- 정확한 세션 시간과 저장·복귀 단위

### 제거 테스트

| 요소 | 제거 시 결과 | 판정 |
|---|---|---|
| 의미 기반 글자 | 일반 스킬 선택으로 퇴행 | `NON_NEGOTIABLE_CORE` |
| 조합 문법 | 주문 설계와 발견 소멸 | `NON_NEGOTIABLE_CORE` |
| 직접 작성 | 마법 사용자 판타지와 주체성 약화 | `NON_NEGOTIABLE_CORE` |
| 상황 반응 | 레시피 암기·화력 비교로 퇴행 | `NON_NEGOTIABLE_CORE` |
| 세계 변화 피드백 | 원인·결과 학습 불가 | `NON_NEGOTIABLE_CORE` |
| 학습·실전·기록 순환 | 학교와 성장이 메뉴 모음으로 분리 | `NON_NEGOTIABLE_CORE` |
| 학교·관계·스톡·소환수 | 코어는 유지되나 깊이·편의·몰입 약화 | `CORE_SUPPORT` |
| 실시간 전투·3년 구조·완성 주문 스톡 | 다른 표현·범위로 교체 가능 | `CHANGEABLE_SHELL / UNPROVEN_OPTION` |

## 9. WHY → HOW → WHAT

### WHY

플레이어가 정답 스킬을 고르는 것이 아니라 마법의 의미를 이해하고 스스로 해결책을 만들었다는 감각을 제공한다.

### HOW

- 첫 5분 안에 직접 작성과 결과 변화를 경험한다.
- 수업에서 글자와 문제의 원리를 배운다.
- 교내 연습에서 직접 작성·조합하여 원인과 결과를 익힌다.
- 현장실습에서 상황과 위험을 관찰한다.
- 메인 글자로 핵심 현상을 선택한다.
- 보조 글자로 형상·전달·출력·조건 등을 변형한다.
- 중요하거나 새로운 구성은 직접 작성한다.
- 이미 숙련한 반복 구성은 스톡으로 줄인다.
- 결과를 즉시 관찰하고 원인과 결과를 학습한다.
- 일상 관계와 정보가 이후 실습의 동행·관점·대화를 바꾼다.
- 필수 이야기는 짧게, 선택한 이야기는 깊게 제공한다.

### WHAT

- 터치 기반 마법 글자 작성
- `메인 글자 1개 + 보조 글자 0개 이상` 문법
- 상황에 따른 주문 조합·변형
- 적·환경·사건의 즉각적 반응
- 수업·교내 연습·현장실습·발견 기록 순환
- 마도서·숙련·하위 글자 스톡
- 코어를 지원하는 마법학교·관계·소환수
- 10~15분 목표의 모바일 세션
- 필수 핵심층과 선택형 상세층의 서사 UX

## 10. 뾰족한 재미

```text
상황과 위험을 읽는다
→ 필요한 현상과 변형을 선택한다
→ 핵심 글자를 직접 작성하고 숙련 반복은 스톡으로 삽입한다
→ 주문 결과가 적·환경·문제를 어떻게 바꾸는지 확인한다
→ 새 활용을 발견하거나 실패 원인을 이해한다
→ 다음 주문을 더 영리하게 설계한다
```

### 반복 행동

- 관찰
- 의미 선택
- 조합
- 직접 작성 또는 스톡 삽입
- 결과 해석
- 재설계

### 핵심 고민

- 지금 필요한 핵심 현상은 무엇인가?
- 어떤 보조 효과가 이 상황을 가장 효율적으로 바꾸는가?
- 직접 작성할 가치가 있는가, 준비한 스톡을 사용할 것인가?
- 짧고 안전한 주문과 복잡하고 강한 주문 중 무엇을 선택할 것인가?

### 목표 감정

- 발견
- 영리함
- 마법을 이해했다는 숙련감
- 내 설계가 실제로 작동했다는 주체성

## 11. 요소 판정

| 요소 | Gate 1 판정 | 이유 | 후속 검증 |
|---|---|---|---|
| 의미 기반 글자·문법 | `NON_NEGOTIABLE_CORE` | 조합 발견의 원인 규칙 | 대표 글자·결과 차이 |
| 상황에 따른 주문 변형 | `NON_NEGOTIABLE_CORE` | 핵심 고민과 재설계 | 적·환경 반응 사례 |
| 직접 터치 작성 | `NON_NEGOTIABLE_CORE` | 마법 사용자 판타지 | 피로·인식·손가림 |
| 즉각적인 현상 변화 | `NON_NEGOTIABLE_CORE` | 원인과 결과 학습 | 피드백 가독성 |
| 수업→연습→실전→기록 | `NON_NEGOTIABLE_CORE` | 배움과 설계의 성장 연결 | CORE_POC 인과 |
| 마법학교 | `CORE_SUPPORT` | 새 문법·문제·전문화 공급 | 최소 표현 범위 |
| 일상 관계 | `CORE_SUPPORT` | 실습 관점·동행·의미 | 제작량 대비 체감 |
| 마도서·발견 기록 | `CORE_SUPPORT` | 발견과 다음 연구 축적 | 기록 UX |
| 하위 글자 스톡 | `CORE_SUPPORT` | 숙련 반복 감소 | 판단 보존 여부 |
| 완성 주문 스톡 | `UNPROVEN_OPTION` | 긴급 대응, 버튼 전투화 위험 | 필요성·사용량 |
| 소환수 | `CORE_SUPPORT_WITH_UNPROVEN_EFFICIENCY` | 작성 시간·동반자 경험 | 화면 혼잡·효용 |
| 실시간 전투 | `CHANGEABLE_SHELL / UNPROVEN_OPTION` | 긴장 또는 사고 방해 가능 | 감속·단계형 비교 |
| 집중 모드 | `CORE_SUPPORT / UNPROVEN_OPTION` | 모바일 작성 압박 조절 | 배율·악용·접근성 |
| 선택형 상세 서사 | `CORE_SUPPORT` | 깊이와 기본 세션 보호 | 제작량·분량 규칙 |
| 3년·6학기 | `CHANGEABLE_SHELL_SCOPE_RISK` | 장기 프레임·범위 위험 | 콘텐츠 제작량 |
| 메인 글자 Lv.0~5 | `CHANGEABLE_SHELL / UNPROVEN_OPTION` | 수치 성장 편향 위험 | 기능 변화 중심 검토 |
| 학년별 보조 2·4·6 | `BALANCE_TUNING_BACKLOG` | 테스트 전 고정 불가 | CORE_POC 이후 |

## 12. 보호할 결정

- 주문은 `메인 글자 1개 + 보조 글자 0개 이상`이다.
- 메인 글자 하나만으로 기본 주문이 성립한다.
- 복수 메인 글자 결합은 금지한다.
- 신규·미숙·중요 글자는 직접 작성한다.
- 숙련 보조만 하위 글자 스톡 후보가 된다.
- 소환수는 회로·글자를 직접 작성·배치·연결하지 않는다.
- 소환수 주문 보조는 전투·수호·스톡 생산·충전 범위다.
- 스톡 없이 직접 시전할 수 있다.
- 조건부 완전 자동 발동을 재도입하지 않는다.
- 필수 정보를 선택형 상세 대화에만 숨기지 않는다.
- 수업은 완성 주문 상점이 아니다.
- 현장실습은 정답 주문 재생 시험이 아니다.
- 일상은 전투와 무관한 호감도 메뉴가 아니다.
- 자유 일정은 필수 진행을 영구 차단하지 않는다.
- 첫 마법 사용 전 장문 설명을 강제하지 않는다.
- 첫 수업을 따라 그리기만 하는 긴 튜토리얼로 만들지 않는다.
- 첫 5분에 모든 전투·스톡·소환수 시스템을 동시에 공개하지 않는다.
- 수동 작성 경험 없이 자동화를 먼저 공개하지 않는다.
- 첫 자동화는 하위 글자 스톡부터 시작한다.
- 의미 기반 글자·직접 작성·상황 변형·세계 변화·학습 순환을 범위 축소하지 않는다.
- 사용자 승인 전 `CORE_CONFIRMED`를 사용하지 않는다.

## 13. 보류와 미검증

`GM-STOCK-01` 세부 수치는 다음 항목 전까지 보류한다.

1. 세일즈포인트와 CORE_POC 범위
2. 모바일 UI·인지 부하 검증
3. 실제 세션·전투 시간 테스트
4. 실시간·감속·단계형 전투 표현 비교

### P1

- 모바일에서 작성·마나·적·스톡·충전·소환수·대상을 동시에 처리할 위험
- 조합 결과가 불명확하면 발견이 아니라 레시피 암기·시행착오 노동이 되는 위험
- 직접 작성이 반복 피로와 화면 가림을 만드는 위험
- 완성 주문 스톡 비중이 높아져 버튼 전투로 퇴행할 위험
- 학교 3년 콘텐츠와 선택형 상세층 제작량이 코어 검증보다 먼저 커질 위험
- 일상 관계가 실습과 연결되지 않으면 별도 호감도 메뉴로 분리될 위험
- 지원 시스템을 코어로 오인하면 범위를 줄이지 못하고 CORE_POC가 비대해질 위험

### BLOCKED_UNVERIFIED

- 첫 마법 발현·첫 독립 설계까지의 실제 시간
- 첫 실습 진입 시점과 난이도
- 첫 숙련·스톡 해금 요구량
- 실제 세션 시간이 10~15분 목표에 맞는지
- 전투·문제 묶음이 모바일 중단 단위로 적합한지
- 전투 중 완전 상태 복원이 필요한지
- 터치 작성의 반복 재미와 인식 허용 범위
- 실시간 압박이 사고를 강화하는지 방해하는지
- 스톡이 조합 선택을 보존하는지
- 소환수 충전 판단이 의미 있는지
- 짧은 주문과 복잡한 주문의 공존
- 실제 모바일 UI 혼잡도
- 기본층과 상세층의 적정 대화 분량
- 자유 일정과 관계 변화가 제작량 대비 충분한 차이를 만드는지
- 비타협 코어만 포함한 CORE_POC가 독립적으로 재미있는지

## 14. Gate 1 진행 순서

```text
GM-CORE-01 — 승인됨
→ GM-AUDIENCE-01 — 승인됨
→ GM-NARRATIVE-03 — 승인됨
→ GM-PROMISE-01 — 승인됨
→ GM-LOOP-01 — 승인됨
→ GM-CURVE-01 — 승인됨
→ GM-CORE-BOUNDARY-01 — 승인됨
→ GM-SALES-01 세일즈포인트 최대 3개
→ 벤치마킹·SWOT·VRIO·제작성
→ 마스코트·상징 동반자 역할 후보
→ CORE_POC 계약
→ 적대적 검토
→ 사용자 Gate 1 승인
```

## 15. 다음 차단 결정 — GM-SALES-01

플레이어와 시장에 즉시 전달할 세일즈포인트를 최대 3개로 압축한다.

> 스펠을 처음 보는 사람이 한 문장과 세 장면만 보고도 이해해야 하는 차별점 3개는 무엇인가?

상태: `DESIGN_HYPOTHESIS_REQUIRES_USER_DECISION`

## 16. Skill Execution Evidence

| Skill | Mode | 결과 | 상태 |
|---|---|---|---|
| `managing-project-intake-and-work-contract` | `route → contract → decompose-and-sequence` | 기존 문서 참조화·v6 원본화·Gate 1 재시작 범위 | `EXECUTED_AND_EVIDENCED` |
| `identifying-project-core` | `read-only-core-identification → removal-test` | 비타협 코어·지원층·외피 분류 | `EXECUTED_AND_EVIDENCED` |
| `analyzing-and-refining-game-concepts` | `frame → constrain → sharpen → structure` | 경험·대상·서사·약속·Loop·경험 곡선·코어 경계 승인 | `EXECUTED_AND_EVIDENCED` |
| `managing-design-documents` | `author/update` | Gate 1 책임 원본·기준선·결정 기록 갱신 | `EXECUTED_AND_EVIDENCED` |
| `establishing-project-core` | 미실행 | Gate 1 전체 코어 승인 전 | `ROUTED_NOT_NEEDED` |
| `designing-vertical-slices` | 미실행 | CORE_POC와 Gate 1 전 | `BLOCKED` |

## 17. 완료·미완료 판정

- 최상위 경험: `APPROVED`
- 목표 플레이어·플레이 상황: `APPROVED`
- 선택형 서사 깊이: `APPROVED`
- 플레이어 약속: `APPROVED_FOR_GATE_1`
- Loop 구조: `APPROVED_FOR_GATE_1`
- 경험 곡선: `APPROVED_FOR_GATE_1`
- 프로젝트 코어 경계: `APPROVED_FOR_GATE_1`
- 세일즈포인트: `NOT_AUTHORED`
- Gate 1 전체: `UNVERIFIED`
- 프로젝트 코어 전체: `CORE_CANDIDATE`
- CORE_POC: `NOT_AUTHORED`
- 구현: `NOT_STARTED`
- 다음 사용자 결정: `GM-SALES-01`
