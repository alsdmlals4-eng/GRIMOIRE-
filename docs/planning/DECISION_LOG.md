...
- 기획적으로 확정된 구조는 `충전 지속`, `진행도 보존`, `동일 상태 중첩 없음`, `재적용 시 지속시간 갱신`이다.
...
| 후보 ID | 질문 | 상태 |
|---|---|---|
| GM-00-33 | 주문 봉인이 직접 회로 시전까지 차단하는 별도 상위 효과가 필요한가 | `UNRESOLVED` |
| GM-00-34 | 수호·동조 능력치가 두 상태를 저항·해제하는가 | `UNRESOLVED` |
