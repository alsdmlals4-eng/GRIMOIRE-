...
- 결정 추가 기록 A~K: `docs/planning/DECISION_LOG_ADDENDUM_2026-07-26*.md`
...
| GM-WORLD-06 | 육성 구조 | 프린세스 메이커식 육성을 결합 | `DIRECTION_CONFIRMED` | 일정 선택이 능력·관계·활용 성향에 누적 영향 |
...
| GM-WORLD-09 | 교육 기간과 학년 구조 | 3년제·학년당 2학기 | `DIRECTION_CONFIRMED` | 총 6학기로 기초→전문화→독립·졸업 성장 구성 |
| GM-WORLD-10 | 학기 일정 선택 단위 | 자유 일정 10회+고정 사건 4회 | `DIRECTION_CONFIRMED` | 14구간 장기 학기, 6학기 총 자유 일정 60회 |
...
- 자동 충전 대상은 한 번에 1종이다.
- 소환수는 전투·수호·지정 스톡 생산 지원을 담당하며 회로 직접 작성은 하지 않는다.
...
- 현장 문제는 처치 외에도 구조·봉인·복구·조사·환경 조작으로 해결한다.
...
- 정규 교육 기간은 3년이며 학년당 2학기, 총 6학기다.
- 학년은 회로 문법·현장 책임·학교 내 위치를 확장하는 큰 성장 단계다.
- 학기는 수업·활동·관계·현장실습을 선택하는 반복 단위다.
- 1학년은 완결된 학교생활·기초 활용 경험을 제공한다.
- 2학년은 전문 분야와 서로 다른 해결 방식의 차이를 본격화한다.
- 3학년은 자율성·책임·문제 복잡도를 확대한다.
...
- 학교생활·관계·연구가 누적되는 거시 학기 템포는 길게 유지한다.
- DDD는 학기를 압축하는 원칙이 아니라 각 활동의 결과와 보상을 빠르게 제공하는 원칙이다.
- 짧은 NPC 대화·학교 탐색·기록 열람은 자유 일정 횟수를 소비하지 않는다.
- 동일 활동은 숙련·관계·교수 과제·조건부 사건·현장실습 준비로 누적 변화한다.
- 60회의 자유 일정을 모두 고유 컷신으로 제작하지 않고 활동 템플릿을 변주한다.
...
- 일반 사건·수업은 `짧은 대사 + 목표 카드`를 사용한다.
- 긴급 임무는 `경고 + 긴급 목표·위험 카드`를 사용한다.
- 감정·관계·승급 장면은 일반 대화창을 사용한다.
- 선택형 상세는 NPC 재대화·질문 목록·조사·기록으로 제공한다.
...
- 참고 작품의 고유 학교·인물·사건·마법 체계는 복제하지 않는다.
