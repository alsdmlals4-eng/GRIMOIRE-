...
- 결정 추가 기록 A~J: `docs/planning/DECISION_LOG_ADDENDUM_2026-07-26*.md`
...
- 1학년은 튜토리얼만 담당하지 않고 완결된 학교생활·기초 활용 경험을 제공한다.
- 2학년은 전문 분야와 서로 다른 해결 방식의 차이를 본격화한다.
- 3학년은 단순 화력보다 자율성·책임·문제 복잡도를 확대한다.
- 1·2학년 말에는 승급 평가, 3학년 말에는 졸업 종합 평가를 진행한다.
- 학기당 활동 수와 정확한 콘텐츠 분량은 후속 데이터 설계에서 확정한다.
