...
스톡 방해 상태이상
→ 충전 속도·봉인·전환·생산 기여 등에 명시적 영향
```

- 지속 피해와 스톡 방해는 독립 효과로 계산한다.
- 하나의 복합 상태가 두 효과를 가질 수 있으나 피해 틱 자체는 정지를 만들지 않는다.
...
| 후보 ID | 질문 | 상태 |
|---|---|---|
| GM-00-31 | 상태이상별 스톡 영향 축을 어떻게 배정하는가 | `UNRESOLVED` |
| GM-00-32 | 충전 봉인 중 완성 스톡 사용을 허용하는가 | `UNRESOLVED` |
| GM-00-33 | 상태이상 중첩과 충전 속도 하한을 어떻게 처리하는가 | `UNRESOLVED` |
| GM-00-34 | 수호·동조 능력치가 스톡 방해를 저항·해제하는가 | `UNRESOLVED` |
