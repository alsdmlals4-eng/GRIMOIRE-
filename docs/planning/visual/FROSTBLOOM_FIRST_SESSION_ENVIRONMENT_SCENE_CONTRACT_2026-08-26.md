...
implementation: PARTIAL_PRODUCT_ROOT_AND_STORY_EVENT_GUIDED_PRACTICUM_BOUND
image_generation: THREE_SOURCE_CANDIDATES_PERSISTED
runtime_export: FIELD_BASE_CURRENT_PRODUCT_ROOT_AND_STORY_EVENT_BOUND__SCHOOL_AND_BATTLE_CANDIDATES_UNBOUND
runtime_validation: HERA_1280X720_PRODUCT_ROOT_AND_STORY_EVENT_RENDER_NO_CLIPPING_DIAGNOSTICS_CLEAN
...
| `GUIDED_FIELD_PRACTICUM` | `bg_greenhouse_field_base` | 배운 글자를 낮은 위험의 현장에 적용해 첫 결과를 만든다. | 관찰 가능한 흐름/압력 이상, 보호 대상 강조, Preview·Target·Cast UI | `StoryEventRoot/EnvironmentBackground`에 기존 바이트 그대로 결속됨; 새 배경 불필요 |
...
현재 main의 Development Product Root와 현재 `StoryEventRoot`는 `bg_greenhouse_field_base`를 각각의 `EnvironmentBackground`로 소비한다. `StoryEventRoot` 결속은 승인된 `GUIDED_FIELD_PRACTICUM` consumer slot만 닫으며, 조사 복귀·별도 Battle Scene·학교 공용 장면을 구현했다는 뜻이 아니다. `bg_school_common`과 `bg_greenhouse_battle_arena`는 current main에 아직 binding이 없다.

```text
FirstSessionClassScene / EnvironmentBackground → bg_school_common
StoryEventRoot / EnvironmentBackground → bg_greenhouse_field_base (current guided practicum runtime binding)
...
사용자 자동 생성 정책으로 세 source candidate와 native-resolution export candidate가 준비됐다. Task11은 `bg_greenhouse_field_base`를 Product Root backdrop으로 binding했고, 2026-09-02에는 동일한 tracked WebP 바이트를 `StoryEventRoot/EnvironmentBackground`에 재사용했다. 두 binding 모두 1280×720 HERA render/diagnostic readback을 남겼다. 이는 조사 복귀·Battle Scene binding, human/device/performance 검증, 또는 final 2560×1440 delivery approval을 뜻하지 않는다.
...
- `bg_greenhouse_field_base`는 Product Root backdrop과 `StoryEventRoot`의 guided practicum background로만 runtime binding되어 있다. school/battle candidate와 조사 복귀 consumer는 아직 unbound로 기록된다.
