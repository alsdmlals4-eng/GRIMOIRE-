...
status: USER_APPROVED_ACTIVE__MIGRATION_IN_PROGRESS
...
notion_status: RETIRED_AS_CURRENT_CANON_AND_WORK_SURFACE__READ_ONLY_MIGRATION_SOURCE
notion_access_policy: CURRENT_NONLEGACY_CONTENT_READ_ONLY_UNTIL_MIGRATION_COMPLETENESS_AUDIT
...
→ 현재성 있는 구조·작업물의 read-only source
→ write, destination readback, 신규 page/database/view 생성, 삭제·archive·external export 금지
→ repository migration audit가 COMPLETE가 된 뒤 historical discovery-only
...
- 사용자는 Notion의 기존 구조와 현재 가치가 있는 작업물이 누락되지 않게 repository로 옮기도록 명시 승인했다. 이 migration window에서만 Notion은 **read-only source**다. 새 Notion 작업, 수정, 동기화, 삭제, archive, export는 계속 금지한다.
...
- migration 완료 조건은 audit에서 모든 nonlegacy source가 `MIGRATED_TO_REPOSITORY`, `EXISTING_REPOSITORY_OWNER_CONFIRMED`, 또는 `USER_DECISION_REQUIRED`로 판정되고, 새 repository destination이 exact readback되는 것이다. 그 뒤 Notion은 `HISTORICAL_DISCOVERY_ONLY__NO_ROUTINE_READ_OR_WRITE`가 된다.
- 이미 repository에 동기화된 W6 preservation, forecast, visual brief, preflight, 그리고 이 feature spec이 현행 owner다.
- future task의 fresh-read는 user instruction → `AGENTS.md` → `START_HERE.md` → `docs/ACTIVE_CONTEXT.md` → active contract → repository domain owner → actual implementation 순이다. migration 완료 전에는 audit에 이름이 있는 Notion source만 read-only로 보충 확인할 수 있고, 완료 후에는 사용자가 다시 허용한 경우에만 historical discovery로 읽을 수 있다.
...
사용자의 직접 결정으로 채택한 것은 **Notion을 더 이상 현재 작업면·정본으로 사용하지 않는다**와 **구형 데이터는 제외하되 기존 Notion의 구조·현재 작업물을 repository로 빠짐없이 옮긴다**는 범위다. 이 결정은 repository-only current canon과 read-only migration window로 기록했고, 향후 master-GDD profile을 실행할지는 별도의 명시 요청에서만 결정한다.
...
solution: Move the active human-facing canon and all current documentation readback duties to repository-native Markdown; use Notion once as a read-only migration source, then preserve it as no-routine-access historical discovery only.
