...
status: USER_SCOPE_APPROVED__BLUEPRINT_AND_CANDIDATE_READY__RUNTIME_STORY_EXTENSION_NOT_IMPLEMENTED
...
- 후보 이미지를 runtime에 바인드하거나 출시 자산으로 승격
...
| 첫 수업 | 계획됨 / `PLANNED` | `FirstClassRoot` 제안 | 직접 글자 쓰기의 안전한 첫 성공을 만든다. |
| 결투 연습 | 계획됨 / `PLANNED` | `DuelPracticumRoot` 제안 | 학생끼리 대응을 배우는 비살상 사건 시계 장면으로 만든다. |
| 축제 | 계획됨 / `PLANNED` | `FestivalCanopyRoot` 제안 | 여러 유효한 돌봄의 표현을 비전투로 마무리한다. |
...
### SCREEN-02 — 공용 스토리 사건 (현재 첫 실습, 수업/결투/축제로 확장)
...
| SIT-002 | 첫 수업 | P0 | 입학식 → 첫 실습 | 안전한 Goal만; 실패가 파국이 되지 않는 관찰 | 수업 레퍼런스 `USER_APPROVED_PLANNING_REFERENCE` |
| SIT-003 | 첫 실습 | P0 | 첫 수업 → 결투 연습 | Goal/Threat 동시, 명시 Preview/Target/Cast | 기존 첫 사건 `IMPLEMENTED`; 온실 레퍼런스 재사용 |
| SIT-004 | 결투 연습 | P1 | 실습 복기 → 축제 | 상대 의도 파악 Goal, 과한 대응/연습장 불안정 Threat | 환경 후보 02 `GENERATED_CANDIDATE` |
| SIT-005 | 축제 | P1 | 결투 복기 → 다음 장 | 공동 복구 Goal, 준비 지연 Threat; 비전투 | 축제 레퍼런스 `USER_APPROVED_PLANNING_REFERENCE` |
...
## 6. Godot 구조와 상태 계약 (구현 계획 입력)

```text
StoryProgress (authoritative story beat + card unlocks)
  ├─ StoryChapterDefinition (planned static route/scene/return contract)
  ├─ FirstClassRoot (planned scene)
  ├─ StoryEventRoot (current first practicum scene, adapted route)
  ├─ DuelPracticumRoot (planned scene)
  └─ FestivalCanopyRoot (planned scene)
...
| 수업 | class direct-glyph planning reference | runtime background는 아직 없음 | `FirstClassRoot/EnvironmentBackground` | planning reference only |
| 실습 | first guided greenhouse reference + 현재 EventClock UI | 기존 사건 화면의 실제 레이어 확인 | `StoryEventRoot` | current runtime + reference |
| 결투 | 신규 cloister 후보 02 | 환경만; 대상/글자/UI는 별도 라이브 레이어 | `DuelPracticumRoot/EnvironmentBackground` | `GENERATED_CANDIDATE` |
| 축제 | festival light-thread canopy reference | runtime background는 아직 없음 | `FestivalCanopyRoot/EnvironmentBackground` | planning reference only |
...
- Godot: 1280×720, 1920×1080에서 긴 한국어·빈/disabled 상태·취소/복귀·중복 입력을 확인한다.
- 후보 이미지: 최종 잠금 전에는 Scene import/bind를 하지 않는다. 사용자가 lock하면 텍스트 없는 배경을 `MOUSE_FILTER_IGNORE`로 바인드하고, 별도 runtime crop 검증을 수행한다.
- Human/Device/Accessibility/Performance/Export: 이 설계와 후보 생성만으로는 `NOT_RUN`이다.

## 9. 완료·다음 단계

이번 문서는 블루프린트·와이어프레임·벤치마크와 후보 준비까지다. 다음 구현은 `docs/superpowers/plans/2026-09-01-story-arc-blueprint-implementation.md`를 따르며, 카드 상세 규칙과 결투 환경 후보의 final lock은 사용자 결정이 있어야만 다음 단계로 간다.
