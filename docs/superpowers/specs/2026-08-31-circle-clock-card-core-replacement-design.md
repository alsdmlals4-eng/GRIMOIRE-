...
├─ 설정
└─ 저장 데이터가 없는 경우, 이어하기를 숨기거나 비활성 사유를 명확히 표시
```

`수업`, `실습`, `결투`, `축제`, `사건 기록`, `마도 카드`를 메인 메뉴의 모드 선택 버튼으로 두지 않는다. 이 요소들은 이야기 장면 안에서 서사 조건을 만족할 때 열리고, 해당 장면의 인물·대화·목표·시계와 함께 제시된다.
...
- 사용자 소유의 실제 저장 데이터가 발견될 경우, 삭제 전의 비파괴적 확인과 별도 처리 결정

저장 데이터가 실제로 존재하는지, 어떤 형식인지, 현재 플레이어에게 배포된 데이터가 있는지는 아직 확인되지 않았다. 구현 전 저장 경로와 유효 데이터를 읽기 전용으로 조사한다. 실제 사용자 데이터가 발견되면 삭제하거나 자동 변환하지 않고, 영향과 복구 선택지를 먼저 보고한다.
...
| 메인 메뉴가 모드 선택 허브가 됨 | 메인 씬은 새 기록/이어하기/설정만 소유 | navigation integration test |
