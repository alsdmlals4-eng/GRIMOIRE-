...
Build a non-product diagnostic package that can determine whether a specific environment is capable of starting the approved Foundation POC work without creating a Godot project or game runtime files.
...
`tools/check_godot_toolchain.py` is a read-only Python CLI.
...
- test a non-project headless invocation that does not require `project.godot`;
...
- claiming renderer, device, or performance quality beyond the probe performed.

### 3.2 Contract tests

`tests/test_foundation_preflight_contract.py` tests parser behavior, state classification, exit codes, redaction, and test-double separation.
...
The stable required check should be a final gate that interprets optional-job states explicitly.
...
exit_5: HEADLESS_FAILED
exit_6: EXPORT_TEMPLATE_MISSING_OR_MISMATCH
exit_7: ANDROID_TOOLCHAIN_BLOCKED
exit_8: INVALID_CONFIGURATION_OR_SCHEMA
```

The CLI must not collapse multiple failures into a false success. JSON records all findings; the exit code represents the highest-priority blocking state.
...
A changed-file guard rejects Stage 0 PRs that touch prohibited product paths.
...
5. Headless and renderer startup evidence were collected.
6. Matching export templates were verified.
7. Android environment status was recorded accurately.
8. iOS host restrictions were recorded accurately.
9. Prohibited product paths were unchanged.
10. Canon freshness and generated-view checks passed.
11. Base v9.4.3 plan read-only revalidation completed.
12. A new Readiness report reached P0=0 and P1=0.
