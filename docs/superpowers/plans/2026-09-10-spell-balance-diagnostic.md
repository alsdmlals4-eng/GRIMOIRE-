...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task in the current task. Do not dispatch agents without applicable authorization. Steps use checkbox (`- [ ]`) syntax for tracking.
...
- [ ] 아래 테스트를 작성한다. 명령 `python -m unittest discover -s tests -p test_spell_balance_diagnostic.py -v`를 실행해 모듈 없음으로 실패하는지 확인한다. 다른 실패면 먼저 원인을 분리한다.
...
- [ ] 같은 명령으로 3 test methods 통과를 확인한다. 9사례와 test method 수를 혼동하지 않는다. 각 함수 docstring에 비음수 내부 진단 입력 전용·생존 미모델링을 명시한다.
- [ ] 두 새 파일만 diff 검토 후 commit한다.

## Task 2: Opening-hand eligibility, not win probability

- [ ] 테스트 import에 `opening_counts` 추가, 아래 method를 추가하고 누락 함수 실패를 확인한다.
...
- [ ] 다음 최소 구현을 추가한다. 중복 물리 카드를 index로 구분하고 종류별 축약 손패를 균등분포로 오인하지 않는다.
...
- [ ] 같은 unittest 명령으로 4 methods 통과 확인. `41`과 `15`는 중첩 가능 집합이며 상호배타 확률이 아님을 기록한다. 초기 손패가 미래 시전을 보장하지 않는다는 주석을 추가한다.
- [ ] 해당 두 파일만 commit한다.

## Task 3: Evidence and stop boundary

- [ ] 전체 새 unittest 실행, `git diff --check`, 프로젝트 operating-contract validator를 실행한다. 실제 성공/실패 수와 stderr를 읽는다.
- [ ] rules 17절과 Active Context에 증거를 기록한다. 기존 Godot suite는 제품 무변경이라 미실행이라고 명시하고 새 규칙 runtime PASS로 바꾸지 않는다.
- [ ] 5회 전체 범위 자체 검토: 수치 owner, 손패 분포, 정책 정보 경계, 소비/저장 비모델링, 제품/진단 증거 분리. 수정 뒤 재검사한다.
- [ ] 문서만 별도 commit 후 현재 branch push/fetch/readback. 기존 dirty fixture와 main 상태를 보고한다.
- [ ] 다음 계획 입력은 제한 결과와 누락 목록이다: 적법 매 교환 손패, 드로우/정돈, 생존/조기 종료, 만료, 동시 승패, 재개 동일성. 이 여섯 항목 없이 승률·전략 우세·최종 수치를 발표하지 않는다.

## Plan self-review

계획만 작성됨: 위 체크박스는 실행 완료가 아니다. 도구 내 9사례 산술 실행은 rules 17절이 소유한다. 41/15는 구현 테스트의 기대값이며 이 계획 작성 시 해당 unittest는 NOT_RUN. 공통 이벤트 엔진과 UI까지 이 계획에 묶지 않는다.
