...
pinned_commit_sha: BLOCKED_PENDING_OFFICIAL_SOURCE_VERIFICATION
download_or_vendor_method: VERIFY_EXISTING_VENDOR_TREE_THEN_VENDOR_PINNED_SOURCE_ONLY_IF_REQUIRED
integrity_hash: BLOCKED_PENDING_SOURCE_TREE_HASH
license: MIT
...
upgrade_process: "new spec PR pins source commit, verifies license/compatibility, runs old/new suites, then updates"
...
known_risks:
  ...
  - "canonical source commit and integrity hash are not yet pinned"
...
```yaml
repository_files: VENDORED_PREEXISTING
...
기존 `addons/gut` 파일은 v4.3 이전에 저장소에 존재했다. 파일 존재를 채택 완료로 해석하지 않는다. 구현 PR은 공식 소스 commit과 현재 vendor tree의 일치 여부를 검증하고, 일치하지 않으면 임의 patch가 아니라 승인된 pinned source로 교체한다.
...
```text
latest merged main
→ source/commit/license/integrity verification
→ RED contract
...
- source·version·license·compatibility·consumer·CI·removal 계획이 명시됨.
- 변경 파일이 문서·명세 계약 테스트·정본 상태에 한정됨.
- `project.godot`, Scene, Resource, asset, GUT 설치 파일 변경 0.
- exact-HEAD checks PASS.
- unresolved thread 0.
- P0/P1 finding 0.
- GPT 검토 역할을 분리한 diff 검토와 사용자 결정권 정책을 충족.
