...
StoryDialogue presentation rows가 S00/S01/S05/S08의 인물별 대사를 소유한다. StoryScreen은 한 번에 한 화자의 대사, 이전/다음 읽기, 별도 안내/실습 기록, 마지막 대사 뒤 명시 선택을 제공한다. CAUSE/RISK 설명 후 주인공 발화→서로 다른 교수 응답→동료 발화로 이어진다. 축제 마무리는 실제 SOLVED/ASSISTED/STOPPED 결과를 따른다. 새 관계 점수·보상·게임 규칙 추가 없음.

대사 위치는 읽기 전용 화면 상태다. 메뉴 왕복에서는 같은 위치 유지, 저장 불러오기/게임 재시작은 해당 장면 첫 대사부터 다시 읽는다. 읽기로 저장/시간/마력/RNG를 바꾸지 않는다. 기존 전체 결과는 별도 실습 기록으로 이동했다. 사용자 승인 캐릭터는 알파/여백/runtime 준비가 남아 있어 이번 연결에서 승격하지 않았다. 실제 인물 portrait/표정/장식 프레임, 사건 화면 설명 통합은 미완료다.

실제 editor11900/runtime34528에서 이어하기→다음 대사(화자 나)→메뉴→복귀→실습 기록→대화 복귀 확인. 1280x720 capture `artifacts/local-validation/story-conversation-20260913.png`, diagnostics0/0. Sorcery 공식 소개의 행동 반응형 서사와 Ren'Py 공식 문서의 화자/대사 분리를 ADAPT. 직접 타게임 플레이·Human/mobile 증거는 아니다. Base9.4.3 pin 유지, origin/main d384c454와 다른 open PR은 읽기 전용 유지.

## 2026-09-13 진행 중 메뉴 및 텍스트 RPG 대화 전환

최신 사용자 방향: 대화·설명창은 시스템 상태창이 아닌 텍스트 RPG로 구성한다. StoryScreen에 장소/짧은 상황 묘사/별도 화자/대사와 명시 선택을 분리하고 기존 남색·황동 기능 테마를 재사용했다. 입학/마무리 제작 상태 문구는 제거했으며 실제 결과/중단/도움 기록은 보존한다. 최종 삽화/장식 프레임 승격은 하지 않았다. 사건 설명/주문 preview 문장의 서사화와 기록의 별도 펼쳐보기는 잔여 작업이다.

진행 중 메뉴는 같은 StoryScreen을 숨겨 보존하고 복귀 시 시전 전 글자/대상/결투 선택을 유지한다. 도감은 현재 메모리 상태를 읽는다. 명시 저장 후 메인 복귀만 미시전 선택을 해제한다. pause12/menu20/storyscreen25 assertions0failures. 실제 editor11900/runtime28040 이어하기→메뉴→플레이 복귀, diagnostics0errors0warnings, story-dialogue-rpg-20260913.png 1280x720 직접 확인. 모바일/최종 UX/전체 게임 완료 증거가 아니다. 마지막 문장 교정 후 회귀 재실행 대상.
