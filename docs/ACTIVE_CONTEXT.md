...
- 장시간 플레이를 강제하지 않는다.
- 시간은 Gate 1 목표 범위이며 실제 수치는 `PLAYTEST_TUNING_REQUIRED`다.
...
- 새 시스템은 `배움→설계→세계 변화` 중 하나 이상을 강화해야 한다.
- 직접 작성은 주문 설계의 주체성을 증명하고 반복 노동이 되지 않아야 한다.
- 스톡과 소환수는 설계 판단을 대체하지 않고 숙련 반복과 시간 압박을 보조한다.
- 학교 콘텐츠는 완성 주문 지급 메뉴가 아니라 새로운 문법과 문제를 학습시키는 장기 프레임이다.
...
1. Micro·Combat·Session·Semester Loop
2. 중단·복귀 지점
3. 경험 곡선
4. CORE_POC 범위
...
승인된 플레이어 약속을 실제 반복 경험으로 만드는 Micro·Combat·Session·Semester Loop를 정한다.
...
→ GM-NARRATIVE-03 — 승인됨
→ GM-PROMISE-01 — 승인됨
→ GM-LOOP-01 Micro·Combat·Session·Semester Loop
→ 경험 곡선
→ 프로젝트 코어·변경 가능한 외피
→ 세일즈포인트 최대 3개
