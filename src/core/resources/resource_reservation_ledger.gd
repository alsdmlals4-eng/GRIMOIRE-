...
func consume_transaction(transaction_id: StringName) -> Dictionary:
    ...
    for record in records:
        ...
        _reservations_by_node.erase(StringName(record.node_id))
    return {"status": &"OK", "consumed_count": records.size()}


func snapshot_state() -> Dictionary:
    return {
        "universal_stock": _stock_pool.to_dict(),
...
func restore_state(data: Dictionary) -> bool:
    var stock_data: Variant = data.get("universal_stock", null)
    ...
    var stock_result = UniversalStockPoolScript.from_dict(stock_data)
    var vault_result = VaultInventoryScript.from_dict(vault_data)
    if stock_result.get("status", &"") != &"OK":
        return false
    if vault_result.get("status", &"") != &"OK":
        return false

    var temp_stock = stock_result.pool
    var temp_vault = vault_result.vault
    ...
    for raw_node_id in reservation_data.keys():
        ...
        if not _record_exists_in(temp_stock, temp_vault, record):
            return false
        if int(record.get("source", -1)) == GlyphResourceTypes.Source.UNIVERSAL_STOCK:
            stock_record_count += 1
        else:
            vault_record_count += 1
        normalized[node_id] = record

    if temp_stock.to_dict().reservations.size() != stock_record_count:
        return false
...
func _vault_circuit_reservation_count(vault) -> int:
    var count := 0
    for slot in vault.to_dict().slots:
        if int(slot.state) == GlyphResourceTypes.VaultSlotState.RESERVED:
            count += 1
    return count
