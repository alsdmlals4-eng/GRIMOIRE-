...
## 최신 이미지·월간 증빙 운영 지시 — 2026-09-14

이미지 모델의 새 분리 자산 제작은 단색 크로마키 배경 원본 생성 → 배경 제거 → 실제 RGBA/가장자리 검수 순서로 진행한다. 캐릭터·소품 색과 겹치지 않는 키 색을 선택하고 RGB 원본과 투명 결과를 함께 보존한다. 기존 RGB 체크무늬 실패본은 재사용하거나 알파 성공본으로 승격하지 않는다. 장면 배경 자체는 별도 배경 자산으로 유지하고, 인물/오브젝트/효과 분리에 이 절차를 적용한다. 후보·최종 승인·런타임 증거 경계는 유지한다. 세부 제작 이력은 `output/imagegen/dialogue-stage-20260913/PRODUCTION_RECORD.md`가 소유한다.

블루프린트와 별도로 프로젝트명 포함 월간 **AI 활용 작업일지·증빙집 PDF**를 `C:\Users\user\Documents\증빙서류\9월 증빙서류`에 추가 발행한다. 이 경로는 사용자가 명시한 파생 증빙 출력 예외이며 프로젝트 소스 이동 지시가 아니다. 실제 작업일/커밋 시각·사후 기록일·캡처일·PDF 발행일과 검증 상태를 분리한다. 원본으로 확인하지 못한 날짜/계정/비용은 미확인으로 남긴다. 생성기 `tools/build_ai_work_evidence.py`, 로컬 원본 목록은 발행 PDF와 같은 이름의 `.sources.json`, 출력 해시는 `.publication.json`. 이 자료는 기획 정본 또는 지정 정산 양식 대체물이 아니다. 개인 대화 발췌·결제·계정 자료는 공개 Git에 넣지 않고 제출하지 않는다. 발행본은 덮어쓰지 않고 정정 사유와 새 버전을 남긴다.
...
## 최신 사용자 재기획 지시 — 2026-09-10

현재 새 작업은 [재기획 접수와 Aseprite 임시 작업 승인](docs/planning/REPLANNING_INTAKE_2026-09-10.md)을 먼저 읽는다. 기획부터 재검토하며 기존 이미지는 새 제작에서 참고자료로만 사용한다. 아래 과거 시각 잠금·단계 기록을 새 제작 승인으로 자동 승계하지 않는다. Aseprite 전용 임시 경로 예외는 해당 문서의 범위에만 적용한다. 기존 실행 코드·저장·자산 삭제나 미병합 PR 흡수는 이 지시로 자동 승인되지 않는다.
...
4. Notion은 현행 정본·작업면이 아니며 `GR-NOTION-MIGRATION-20260828-01`은 main readback까지 완료됐다. Notion은 historical discovery-only이고 routine read/write, destination readback, 신규 작업면, delete/archive/export는 금지한다. 새 read는 사용자가 명시 허용한 discovery에만 쓴다.
...
13. 이번 승인 범위의 W6 decision-loop L3 구현과 delivery는 완료됐다. 사용자는 2026-08-29 Issue #242 구현을 명시 승인했고, PR #245의 exact head `b0f34b5`는 적용 가능한 required checks를 통과한 뒤 squash merge `713bfbf`로 main에 반영됐다. Human, device, performance, accessibility, export, full-slice는 별도 evidence gate로 남는다. Task8 recovery, W7, runtime production asset batch는 범위 밖이다. Visual 후보는 2026-08-28 사용자 사전 승인에 따라 아래 후보 생성 규칙 안에서 생성할 수 있으나, final lock·runtime 사용·production batch 확장은 사용자 확정이 필요하다.
