...
func run(case) -> void:
	...
	case.assert_true(state.select_target(&"REPAIR"), "a changed target returns from preview to explicit target selection")
	case.assert_equal(State.State.SPELL_USE_TARGET, state.current_state(), "changed target invalidates final preview state")
	case.assert_true(state.final_preview().is_empty(), "changed target clears the previous final preview")
	case.assert_true(state.show_final_preview({"success_percent": 81, "final_mana": 8}), "changed target can request a new final preview")
	case.assert_true(state.request_use_confirmation(), "new preview can request use confirmation")
	case.assert_true(state.recover_failed_use(), "failed use returns explicitly to the retained final preview")
	case.assert_equal(State.State.SPELL_USE_PREVIEW, state.current_state(), "failed use recovery keeps selection context at final preview")
