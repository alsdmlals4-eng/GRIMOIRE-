...
func select_target(target_keyword: StringName) -> bool:
	if (_state != State.SPELL_USE_TARGET and _state != State.SPELL_USE_PREVIEW) or target_keyword.is_empty():
		return false
...
func recover_failed_use() -> bool:
	return _transition(State.SPELL_USE_CONFIRM, State.SPELL_USE_PREVIEW)
