...
## 기본 실행: 이야기 메인 메뉴 — 2026-09-13

최신 설정: 대화 본문 크기24/28/32px 저장·복원과 실제 본문 적용까지 연결했다. 사건·결투 확대, 음량·모션은 미완료이며 부분 설정으로 표시한다. 도감도 현재 연결됨. 초기 메뉴의 설정/도감 미구현 표기는 아래 역사 단계로만 읽는다.

후속: 도감은 배운 네 글자 뜻과 실제 사용 맥락을 보여주는 읽기 전용 화면으로 연결됐다. 설정/최종 아트/진행 중 도감 진입은 별도 잔여 작업이다. 아래 초기 메뉴의 도감 미구현 표기는 이전 단계 기록이다.

Godot 기본 실행은 `res://src/ui/story/story_menu.tscn`. 새 게임/기존 기록 대체 확인/이어하기/마지막 기록의 메인 복귀를 연결한다. 설정·도감·최종 아트는 미완료로 명시한다. 이전 Product Root는 독립 회귀용으로 보존하며 아래 기본 main 미변경 기록은 과거 시점이다. 최신 증거는 Active Context 상단, 계획은 `docs/superpowers/plans/2026-09-13-story-main-entry.md`.

## 현재 이야기 연결 진입 — 2026-09-13

최신 확장: 위 새 씬은 이제 S00~S08, 입학→수업→안내 결투→온실→복기 설명→후속 시료 실습→축제 준비→결과 기록까지 기능 연결됐다. LAB_SAMPLE_02는 별도 사건으로 추가했고 기본 세 사건 시험 순서는 유지한다. 1038 자동검사/실제PC 연결 실행 확인. 최종 그림/프로필/메인복귀/모바일/장편게임 완료는 아니다. 아래 S05까지의 설명은 이전 시점이다.
