...
ENTRYPOINTS = (
    ROOT / "AGENTS.md", ROOT / "START_HERE.md", ROOT / "docs/ACTIVE_CONTEXT.md",
...
class CurrentCheckpointAuthorityContractTests(unittest.TestCase):
    ...
    def test_active_entrypoints_route_to_current_checkpoint(self) -> None:
        ...
        required = (
            "working_pull_request: 67", "working_branch: agent/gr-test-032-human-execution-pack",
    ...
    def test_machine_state_tracks_same_sync_and_preserves_gates(self) -> None:
        ...
        self.assertEqual(bundle["headless_suite_count"], 28); self.assertEqual(bundle["headless_assertion_count"], 1343)
        self.assertEqual(bundle["headless_failure_count"], 0); self.assertEqual(bundle["runtime_glyph_count"], 6)
        self.assertEqual(bundle["planned_participant_count"], 6); self.assertEqual(bundle["planned_total_scored_attempts"], 144)
        ...
        self.assertFalse(bundle["human_pass_claimed"]); self.assertFalse(bundle["merge_authorized"])

        current_work = grill["current_work"]
        self.assertEqual(grill["current_count"], 4); self.assertEqual(current_work["pull_request"], 67)
        self.assertEqual(current_work["working_branch"], "agent/gr-test-032-human-execution-pack")
        self.assertEqual(current_work["parent_pull_request"], 65); self.assertEqual(current_work["gate"], GATE)
        ...
        self.assertFalse(current_work["human_pass_claimed"]); self.assertFalse(current_work["merge_authorized"])

    def test_checkpoint_keeps_core_fun_and_human_boundaries(self) -> None:
        text = "\n".join(path.read_text(encoding="utf-8") for path in
                         (CURRENT_CHECKPOINT, CURRENT_STATUS, READY_GATE, SYNC_RECEIPT) if path.is_file())
        for token in (
    ...
    def test_previous_harness_and_execution_pack_tdd_evidence_is_preserved(self) -> None:
        sync = json.loads(CANON_SYNC_STATE.read_text(encoding="utf-8")); bundle = sync["current_bundle"]
        tdd = bundle["tdd"]
        ...
        self.assertEqual(tdd["authority_red_head"], "d65d0c59d0a75356e6209e6faf4afec868a5f463")
        self.assertEqual(tdd["authority_red_run"], 31025447928)
        self.assertEqual(tdd["raw_evidence_red_head"], "715abd34bf079640ff85c61ebc14df66a33a971f")
        self.assertEqual(tdd["raw_evidence_red_run"], 31026128217)
        self.assertEqual(tdd["raw_evidence_green_head"], "7bc761af330f671386a518fa2736c682c3bca53b")
        self.assertEqual(tdd["raw_evidence_green_run"], 31026381453)
        self.assertEqual(tdd["authority_preservation_red_head"], "f77a22e353bd79a79f02c32a70dbfb73fb7b8a15")
        self.assertEqual(tdd["authority_preservation_red_run"], 31026511615)
        self.assertEqual(tdd["authority_preservation_green_head"], "dd59261bf7b9b11f628dda05e1d875888a99f8d1")
        self.assertEqual(tdd["authority_preservation_foundation_run"], 31026878447)
        self.assertEqual(tdd["authority_preservation_planning_run"], 31026878460)
        self.assertEqual(tdd["result"], "PASS_PENDING_EXACT_HEAD_CLOSURE")
