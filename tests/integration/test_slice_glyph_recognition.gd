...
func run(case) -> void:
    ...
    for glyph_id in [&"HEAT", &"PROTECT", &"FLOW", &"FOCUS", &"DISPERSE", &"BURST"]:
        ...
        var row := _recognize_row(service, String(glyph_id).to_lower() + "-positive", glyph_id, strokes, revision, "developer_reference")
        rows.append(row)
    ...
    var swapped := _load_strokes("res://tests/fixtures/glyphs/heat-stroke-order-swapped.json")
    var swapped_row := _recognize_row(service, "heat-stroke-order-swapped", &"HEAT", swapped, revision, "allowed_stroke_order")
    ...
    var incomplete_row := _recognize_row(service, "heat-incomplete", &"HEAT", incomplete, revision, "incomplete")
    ...
    for confusing_fixture in [
        "res://tests/fixtures/glyphs/heat-light-confusing.json",
        "res://tests/fixtures/glyphs/flow-repeat-confusing.json",
    ]:
        var confusing := _load_strokes(confusing_fixture)
        var fixture_id := confusing_fixture.get_file().get_basename()
        var confusing_row := _recognize_row(service, fixture_id, &"", confusing, revision, "confusing_unknown")
    ...
    var unknown_row := _recognize_row(service, "unknown-zigzag", &"", unknown, revision, "unknown")
