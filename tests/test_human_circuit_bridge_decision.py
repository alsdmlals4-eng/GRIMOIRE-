...
class HumanCircuitBridgeDecisionContractTests(unittest.TestCase):
    def test_required_decision_artifacts_exist(self):
        for path in (APPROVAL, PROTOCOL, ADVERSARIAL): self.assertTrue(path.is_file(), path)

    def test_approved_option_is_staged_hybrid(self):
        text = "\n".join(path.read_text(encoding="utf-8") for path in (APPROVAL, PROTOCOL, ADVERSARIAL, CURRENT_DECISIONS))
        for token in (DECISION_ID, "C_STAGED_RECOGNITION_THEN_CORE_LOOP", "STAGE_1_RECOGNITION_DIAGNOSTIC", "STAGE_2_CORE_LOOP_BRIDGE", "SAME_PARTICIPANT_WITH_BREAK", "FIRST_ATTEMPT_AND_POST_FEEDBACK_SEPARATED", "NOT_A_FULL_VERTICAL_SLICE_REPRESENTATIVENESS_PASS", "HUMAN_EXECUTION_NOT_RUN"):
            self.assertIn(token, text)

    def test_platform_and_accessibility_benchmarks_are_explicit(self):
        text = PROTOCOL.read_text(encoding="utf-8") + ADVERSARIAL.read_text(encoding="utf-8")
        for token in ("ANDROID_TOUCH_TARGET_48DP", "IOS_TOUCH_TARGET_44PT", "WCAG_2_2_TARGET_SIZE_MINIMUM_24_CSS_PX", "NON_DRAG_ALTERNATIVE_REQUIRED", "PROJECT_USES_STRICTER_PLATFORM_TARGETS"):
            self.assertIn(token, text)

    def test_stage_gates_preserve_product_boundaries(self):
        text = "\n".join(path.read_text(encoding="utf-8") for path in (APPROVAL, PROTOCOL, ADVERSARIAL, DEVELOPMENT_GATES))
        for token in ("ACCEPTED_FALSE_ZERO_HARD_STOP", "ACCIDENTAL_COMMIT_ZERO_HARD_STOP", "RUNTIME_EXPANSION_7_PLUS_BLOCKED", "ALL_NUMERIC_THRESHOLDS_ARE_TEST_VALUE", "STAGE2_HARNESS_AUTOMATED_PASS_HUMAN_NOT_RUN", "HUMAN_DEVICE_VALIDATION_NOT_RUN", "HUMAN_END_TO_END_CORE_LOOP_NOT_RUN"):
            self.assertIn(token, text)

    def test_grill_counter_and_current_sync_preserve_decision_without_merge(self):
        grill = json.loads(GRILL_STATE.read_text(encoding="utf-8")); sync = json.loads(CANON_SYNC_STATE.read_text(encoding="utf-8"))
        ...
        self.assertFalse(grill["counter"]["merge_required"]); self.assertFalse(grill["counter"]["merge_authorized"])
        bundle = sync["current_bundle"]
        self.assertEqual(bundle["sync_id"], CURRENT_SYNC)
