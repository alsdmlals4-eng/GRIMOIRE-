...
class HumanCircuitBridgeDecisionContractTests(unittest.TestCase):
    ...
    def test_platform_and_accessibility_benchmarks_are_explicit(self) -> None:
        text = "\n".join(
            path.read_text(encoding="utf-8")
            for path in (PROTOCOL, ADVERSARIAL)
        )
        required = (
    ...
    def test_grill_counter_and_sync_state_advance_without_merge(self) -> None:
        ...
        self.assertEqual(
            grill["current_work"]["human_validation_protocol"],
            "STAGED_HYBRID_APPROVED_NOT_RUN",
        )
        self.assertFalse(grill["counter"]["merge_required"])
        self.assertFalse(grill["counter"]["merge_authorized"])

        current_bundle = sync["current_bundle"]
        self.assertEqual(
            current_bundle["sync_id"],
            "GR-SYNC-20260805-05-GLYPH-HUMAN-CIRCUIT-BRIDGE",
        )
        self.assertIn(DECISION_ID, current_bundle["decision_ids"])
