...
class GrTest032ExecutionPackContractTests(unittest.TestCase):
    ...
    def test_required_execution_pack_files_exist(self) -> None:
        for path in (RUNBOOK, MODERATOR, PREFLIGHT, SESSION_TEMPLATE, ATTEMPT_SCHEMA, RESULT_TEMPLATE):
            self.assertTrue(path.is_file(), path)

    def test_runbook_preserves_approved_scope_and_honest_boundary(self) -> None:
        text = self._read_required(RUNBOOK)
        required = (
            "GM-GLYPH-HUMAN-CIRCUIT-BRIDGE-01", "GR-TEST-032",
            "READY_FOR_HUMAN_EXECUTION_HUMAN_NOT_RUN", "C_STAGED_RECOGNITION_THEN_CORE_LOOP",
            "STAGE_1_RECOGNITION_DIAGNOSTIC", "SAME_PARTICIPANT_WITH_BREAK",
            "STAGE_2_CORE_LOOP_BRIDGE", "FIRST_ATTEMPT_AND_POST_FEEDBACK_SEPARATED",
            "participants: 6", "break_between_stages_minutes: 10",
            "scored_attempts_per_participant: 24", "planned_total_scored_attempts: 144",
            "EXACTLY_6_RUNTIME_GLYPHS", "SOURCE_ATTEMPT_SHA256_REQUIRED",
            "SOURCE_STAGE2_SHA256_REQUIRED", "SOURCE_RECORD_COUNT_MUST_MATCH_AGGREGATE",
            "HUMAN_DEVICE_VALIDATION_NOT_RUN", "HUMAN_END_TO_END_CORE_LOOP_NOT_RUN",
            "FULL_VERTICAL_SLICE_REPRESENTATIVENESS_NOT_RUN", "RUNTIME_EXPANSION_7_PLUS_BLOCKED",
    ...
    def test_moderator_script_does_not_leak_solution_or_score(self) -> None:
        ...
        for token in ("NO_SOLUTION_PROMPT", "NO_BEST_GLYPH_RECOMMENDATION",
                      "NO_PARTICIPANT_RANKING_OR_SCORE_FEEDBACK", "MODERATOR_INTERVENTION_EVENT",
                      "STOP_ON_HARD_STOP", "ANONYMOUS_PARTICIPANT_ID_ONLY"):
            self.assertIn(token, text)

    def test_device_preflight_covers_required_mobile_conditions(self) -> None:
        text = self._read_required(PREFLIGHT)
        for token in ("NARROW_LANDSCAPE_16_9_TO_18_9", "TALL_LANDSCAPE_19_5_9_TO_20_9",
                      "ANDROID_TOUCH_TARGET_48DP", "IOS_TOUCH_TARGET_44PT",
                      "NON_DRAG_ALTERNATIVE_REQUIRED", "REDUCED_MOTION", "SAFE_AREA",
    ...
    def test_templates_are_empty_not_run_and_pii_safe(self) -> None:
        ...
        self.assertEqual(session["runtime_glyph_ids"], ["HEAT", "PROTECT", "FLOW", "FOCUS", "DISPERSE", "BURST"])
        self.assertEqual(session["planned_participant_count"], 6)
        self.assertEqual(session["planned_total_scored_attempts"], 144)
        self.assertIsNone(session["source_attempt_record_count"])
        self.assertIsNone(session["source_attempt_sha256"])
        self.assertIsNone(session["source_stage2_observation_count"])
        self.assertIsNone(session["source_stage2_sha256"])
        ...
        for token in ("first_attempt", "post_feedback", "moderator_intervention", "artifact_sha"):
            self.assertIn(token, attempt["required_fields"])
        ...
        self.assertIsNone(result["source_attempt_record_count"])
        self.assertIsNone(result["source_attempt_sha256"])
        self.assertIsNone(result["source_stage2_observation_count"])
        self.assertIsNone(result["source_stage2_sha256"])
