...
func run(case) -> void:
	...
	var final = coordinator.prepare_target_preview(&"WARD", {"difficulty": 4, "mana_cost": 3, "target_valid": true}, {"effect": &"PROTECT_WARD"})
	case.assert_equal(&"FINAL_PREVIEW_READY", final.status, "selected target produces Stage 3 final preview")
	case.assert_equal(&"WARD", final.preview.target_keyword, "final preview carries selected target keyword")
